...
@everywhere function my_pdf(x::NTuple{2,Float64},m::Vector{Float64},S::Matrix{Float64})
	dx = x[1] - m[1]
	dy = x[2] - m[2]
...
@everywhere function my_pdf(x::VF, m::VF, S::MF)
	k = length(x)
	xm = x - m
...
@everywhere function fast_pdf(x::VF, m::VF, inv_S::MF, den::Float64)
	xm = x - m
	return exp(-0.5 * dot(xm, inv_S*xm)) / den
end


@everywhere function centroid(d::Matrix{Float64}, L::Float64)
	x_val = 0.0; y_val = 0.0
	n = size(d,1)
	cell_size = L / n
	d_sum = 0.0
...
@everywhere function covariance(d::Matrix{Float64}, L::Float64)
...
@everywhere function directional_derivative(g_f1::VVF, g_f2::VVF, u1::VVF, u2::VVF)
	N = length(g_f2)
	dd = 0.0
	for i = 1:N
...
@everywhere function directional_derivative(g_f1::Matrix{Float64}, g_f2::Matrix{Float64}, u1::VVF, u2::VVF)
	N = size(g_f2, 2)
	dd = 0.0
	for i = 1:N
		dd += dot(g_f1[:,i], u1[i]) + dot(g_f2[:,i], u2[i])
...
@everywhere function directional_derivative(g_f1::Matrix{Float64}, g_f2::Matrix{Float64}, u1::Matrix{Float64}, u2::Matrix{Float64})
	#N = length(g_f1)
	N = size(g_f1, 2)
	dd = 0.0
	for i = 1:N
...
@everywhere function scaled_dd(g_f1::VVF, g_f2::VVF, u1::VVF, u2::VVF)
	dd = directional_derivative(g_f1, g_f2, u1, u2)
	norm_factor = sqrt( dot(u1,u1) + dot(u2,u2) )
	return dd / norm_factor
end

@everywhere function scaled_dd(g_f1::Matrix{Float64}, g_f2::Matrix{Float64}, u1::VVF, u2::VVF)
	dd = directional_derivative(g_f1, g_f2, u1, u2)
	norm_factor = sqrt( dot(u1,u1) + dot(u2,u2) )
	return dd / norm_factor
end

@everywhere function scaled_dd(g_f1::Matrix{Float64}, g_f2::Matrix{Float64}, u1::Matrix{Float64}, u2::Matrix{Float64})
	dd = directional_derivative(g_f1, g_f2, u1, u2)
...
@everywhere function normalize!(mat::Matrix{Float64}, dA::Float64)
...
@everywhere function normalize!(mat::Array{Float64,3}, dV::Float64)
...
# """
@everywhere function mat2traj(mat::Matrix{Float64})
	N,n = size(mat)
...
# """
@everywhere function traj2mat(traj::VVF)
	N = length(traj)
	n = length(traj[1])
	mat = zeros(N, n)
	for i = 1:N
...
export vvvf2vvf
@everywhere function vvvf2vvf(xd::VVVF, ud::VVVF)
	N = length(ud[1])
	xdcat = hcat(xd...)
	udcat = hcat(ud...)
	x = VVF(N+1)
