...
function run_traj_opt_mpc(gpm, replan_rate, N, h, sample_cost, f_prior, max_iters, verbose, logging)
    ...
    while t < N
        if i >= replan_rate
            ...
            if any(1 .== drill_idx) # the next location is a drill
                sample_action_xy = convert_perc2xy(gpm, tm, first(best_sample_actions[1 .== drill_idx]), best_xd)
            ...
            gpm.GP = post_GP #GaussianProcessManagerR2(gpm.domain, post_GP, gpm.σ_drill, gpm.σ_spec)
    ...
    anim = @animate for i ∈ 1:length(t_indexes)#i ∈ 1:(size(ν_storage)[3]-1)
        ...
        dx = (x_max(gpm) - x_min(gpm))/(gpm.domain.cells[1]-1)
        dy = (y_max(gpm) - y_min(gpm))/(gpm.domain.cells[2]-1)
        x = [xd[i][1] for i in 1:length(xd)]
        y = [xd[i][2] for i in 1:length(xd)]
        contourf(0:dx:1, 0:dy:1, reshape(vec(ν), (gpm.domain.cells[1], gpm.domain.cells[2]))', colorbar = true, c = cgrad(:inferno, rev = true), xlims = (0, 1), ylims = (0, 1),legend = false,  xlabel = "x₁", ylabel = "x₂", aspectratio = :equal, clim=(0,1))
    ...
    JLD2.save(path_name * "/GP_PTO/executed_traj.jld", "executed_traj", executed_traj)
    JLD2.save(path_name * "/GP_PTO/executed_drill_locations.jld", "executed_drill_locations", executed_drill_locations)
    JLD2.save(path_name * "/GP_PTO/gp_hist/final_gp_X.jld", "final_gp_X", post_GP.X)
    JLD2.save(path_name * "/GP_PTO/gp_hist/final_gp_y.jld", "final_gp_y", post_GP.y)
    JLD2.save(path_name * "/GP_PTO/gp_hist/final_gp_ν.jld", "final_gp_ν", post_GP.ν)

end

d = Domain(domain_min, domain_max, domain_bins)
run_traj_opt_mpc(GaussianProcessManagerR2(d, GP, σ_drill, σ_spec), replan_rate, N, h, sample_cost, f_prior, max_iters, verbose, logging)
