...
@everywhere mutable struct Domain
	# user provided
	...
	@everywhere function Domain(mins, maxes, num_cells::Int)
		n = length(mins)
		return Domain(mins, maxes, num_cells*ones(Int, n))
	end

	# Allow user to only give maxes
	@everywhere function Domain(maxes, bins::Vector{Int})
		return Domain(zeros(length(maxes)), maxes, bins)
	end

	@everywhere function Domain(maxes, num_cells::Int)
...
@everywhere x_min(domain::Domain) = domain.mins[1]
@everywhere y_min(domain::Domain) = domain.mins[2]
@everywhere z_min(domain::Domain) = domain.mins[3]

@everywhere x_max(domain::Domain) = domain.maxes[1]
@everywhere y_max(domain::Domain) = domain.maxes[2]
@everywhere z_max(domain::Domain) = domain.maxes[3]

@everywhere x_size(domain::Domain) = domain.cell_lengths[1]
@everywhere y_size(domain::Domain) = domain.cell_lengths[2]
@everywhere z_size(domain::Domain) = domain.cell_lengths[3]

@everywhere x_cells(domain::Domain) = domain.cells[1]
@everywhere y_cells(domain::Domain) = domain.cells[2]
@everywhere z_cells(domain::Domain) = domain.cells[3]
