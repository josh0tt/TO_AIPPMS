...
function pto_trajectory(em::ErgodicManager, tm::TrajectoryManager, xd0::VVF, ud0::VVF; verbose::Bool=true, logging::Bool=false, max_iters::Int=100, es_crit::Float64=0.0, dd_crit::Float64=1e-6)
	...
	i = 1
	not_finished = true
	es = 0.; xfs =0.; cs = 0.; ts = 0.; dd = 0.; step_size = 0.; es_diff=0.0
	es_prev = 0.0
	es_count = 1
	while not_finished
		...
		es, xfs, cs, ts = all_scores(em, tm, xd, ud)
		dd = directional_derivative(ad, bd, zd, vd)
		if verbose; step_report(i, es, xfs, cs, ts, dd, step_size); end
	...
	if verbose
		print_header()
		if verbose; step_report(i-1, es, xfs, cs, ts, dd, step_size); end
	end
