@everywhere mutable struct GaussianProcess
    m # mean
...
@everywhere μ(X, m) = [m(x) for x in X]
...
@everywhere Σ(X, k) = kernelmatrix(k, X, X)
@everywhere K(X, X′, k) = kernelmatrix(k, X, X′)
# ν(X, K) = [variance(x, X, K) for x in X]

@everywhere function mvnrand(rng, μ, Σ, inflation=1e-6)
    N = MvNormal(μ, Σ + inflation*I)
    #N = MvNormal(μ, Σ)
    return rand(rng, N)
end
@everywhere Base.rand(rng, GP, X) = mvnrand(rng, μ(X, GP.m), Σ(X, GP.k))
@everywhere Base.rand(rng, GP, μ_calc, Σ_calc) = mvnrand(rng, μ_calc, Σ_calc)
 
@everywhere function query_no_data(GP::GaussianProcess)
    μₚ = GP.mXq
    S = GP.KXqXq
...
@everywhere function query(GP::GaussianProcess)
    # tmp = GP.KXqX / (GP.KXX + diagm(GP.ν))
    tmp = GP.KXqX / (GP.KXX + Diagonal(GP.ν .+ eps()))
...
@everywhere function posterior(GP::GaussianProcess, X_samp, y_samp, ν_samp)
    if GP.X == []
...
@everywhere prob_of_improvement(y_min, μ, σ) = cdf(Normal(μ, σ), y_min) 

@everywhere function expected_improvement(y_min, μ, σ)
