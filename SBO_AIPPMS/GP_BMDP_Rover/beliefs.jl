...
function update_belief(pomdp::P, b::RoverBelief, a::Symbol, o::Float64, rng::RNG) where {P <: POMDPs.POMDP, RNG <: AbstractRNG}
    ...
    μₚ, νₚ, S, EI = b.location_belief.X == [] ? query_no_data(b.location_belief) : query(b.location_belief)
    o = a == :drill ? μₚ[b.pos] : round(rand(rng, Normal(μₚ[new_pos], pomdp.σ_spec)), digits=1)
    ####################################################################
...
function update_belief_no_obs_override(pomdp::P, b::RoverBelief, a::Symbol, o::Float64, rng::RNG) where {P <: POMDPs.POMDP, RNG <: AbstractRNG}
    if isterminal(pomdp, b)
        return b
    end
