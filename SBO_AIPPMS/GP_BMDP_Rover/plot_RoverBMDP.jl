...
function plot_trial(true_map, state_hist, gp_hist, action_hist, total_reward_hist, reward_hist, trial_num, trial_name, use_ssh_dir)
	...
	if use_ssh_dir
		Plots.gif(anim, "/data/figures/GP_BMDP_Rover/$(trial_name)/variance/GP_BMDP_ROVER$(trial_num).gif", fps = 2)
	else
		Plots.gif(anim, "/data/figures/GP_BMDP_Rover/$(trial_name)/variance/GP_BMDP_ROVER$(trial_num).gif", fps = 2)
	end
	...
	if use_ssh_dir
		savefig("/data/figures/GP_BMDP_Rover/$(trial_name)/variance/GP_BMDP_ROVER$(trial_num).pdf")
	else
		savefig("/data/figures/GP_BMDP_Rover/$(trial_name)/variance/GP_BMDP_ROVER$(trial_num).pdf")
	end
...
function plot_trial_with_mean(true_map, state_hist, gp_hist, action_hist, total_reward_hist, reward_hist, trial_num, trial_name, use_ssh_dir)
	...
	if use_ssh_dir
		Plots.gif(anim, "/data/figures/GP_BMDP_Rover/$(trial_name)/mean/GP_BMDP_ROVER$(trial_num).gif", fps = 2)
	else
		Plots.gif(anim, "/data/figures/GP_BMDP_Rover/$(trial_name)/mean/GP_BMDP_ROVER$(trial_num).gif", fps = 2)
	end
	...
	if use_ssh_dir
		savefig("/data/figures/GP_BMDP_Rover/$(trial_name)/mean/GP_BMDP_ROVER$(trial_num).pdf")
	else
		savefig("/data/figures/GP_BMDP_Rover/$(trial_name)/mean/GP_BMDP_ROVER$(trial_num).pdf")
	end
...
function plot_true_map(true_map, trial_num, trial_name, use_ssh_dir)
	...
	if use_ssh_dir
		savefig("/data/figures/GP_BMDP_Rover/$(trial_name)/true_map/true_map$(trial_num).png")
	else
		savefig("/data/figures/GP_BMDP_Rover/$(trial_name)/true_map/true_map$(trial_num).png")
	end
end
...
function plot_error_map(true_map, state_hist, gp_hist, action_hist, total_reward_hist, reward_hist, trial_num, trial_name, use_ssh_dir)
	...
	if use_ssh_dir
		Plots.gif(anim, "/data/figures/GP_BMDP_Rover/$(trial_name)/error_map/error_map$(trial_num).gif", fps = 2)
	else
		Plots.gif(anim, "/data/figures/GP_BMDP_Rover/$(trial_name)/error_map/error_map$(trial_num).gif", fps = 2)
	end
end
...
function plot_RMSE_trajectory(true_map, state_hist, gp_hist, action_hist, total_reward_hist, reward_hist, trial_num, trial_name, use_ssh_dir)
	...
	if use_ssh_dir
		Plots.gif(anim, "/data/figures/GP_BMDP_Rover/$(trial_name)/RMSE_traj/RMSE_traj$(trial_num).gif", fps = 2)
	else
		Plots.gif(anim, "/data/figures/GP_BMDP_Rover/$(trial_name)/RMSE_traj/RMSE_traj$(trial_num).gif", fps = 2)
	end
end
...
function plot_RMSE_trajectory_history(rmse_hist, trial_name, use_ssh_dir)
	...
	if use_ssh_dir
		savefig("/data/figures/RMSE_traj_$(trial_name).pdf")
	else
		savefig("/data/figures/RMSE_traj_$(trial_name).pdf")
	end
...
function plot_trace_trajectory(true_map, state_hist, gp_hist, action_hist, total_reward_hist, reward_hist, trial_num, trial_name, use_ssh_dir)
	...
	if use_ssh_dir
		Plots.gif(anim, "/data/figures/GP_BMDP_Rover/$(trial_name)/trace_traj/trace_traj$(trial_num).gif", fps = 2)
	else
		Plots.gif(anim, "/data/figures/GP_BMDP_Rover/$(trial_name)/trace_traj/trace_traj$(trial_num).gif", fps = 2)
	end
end
...
function plot_trace_Σ_history(trace_hist, trial_name, use_ssh_dir)
	...
	if use_ssh_dir
		savefig("/data/figures/trace_traj_$(trial_name).pdf")
	else
		savefig("/data/figures/trace_traj_$(trial_name).pdf")
	end
