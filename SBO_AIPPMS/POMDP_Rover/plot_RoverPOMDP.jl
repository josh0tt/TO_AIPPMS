...
function plot_trial(true_map, state_hist, belief_hist, action_hist, total_reward_hist, reward_hist, trial_num, solver_name, use_ssh_dir)
	...
	if use_ssh_dir
		Plots.gif(anim, path_name * "/figures/POMDP_ROVER/$(solver_name)/variance/POMDP_ROVER$(trial_num).gif", fps = 1)
	else
		Plots.gif(anim, path_name * "/figures/POMDP_ROVER/$(solver_name)/variance/POMDP_ROVER$(trial_num).gif", fps = 1)
	end
	...
	if use_ssh_dir
		savefig(path_name * "/figures/POMDP_Rover/$(solver_name)/variance/POMDP_ROVER$(trial_num).pdf")
	else
		savefig(path_name * "/figures/POMDP_Rover/$(solver_name)/variance/POMDP_ROVER$(trial_num).pdf")
	end
...
function plot_trial_with_mean(true_map, state_hist, belief_hist, action_hist, total_reward_hist, reward_hist, trial_num, solver_name, use_ssh_dir)
	...
	if use_ssh_dir
		Plots.gif(anim, path_name * "/figures/POMDP_Rover/$(solver_name)/mean/POMDP_ROVER$(trial_num).gif", fps = 1)
	else
		Plots.gif(anim, path_name * "/figures/POMDP_Rover/$(solver_name)/mean/POMDP_ROVER$(trial_num).gif", fps = 1)
	end
	...
	if use_ssh_dir
		savefig(path_name * "/figures/POMDP_Rover/$(solver_name)/mean/POMDP_ROVER$(trial_num).pdf")
	else
		savefig(path_name * "/figures/POMDP_Rover/$(solver_name)/mean/POMDP_ROVER$(trial_num).pdf")
	end
...
function plot_RMSE_trajectory_history(rmse_hist, trial_name, use_ssh_dir)
	...
	if use_ssh_dir
		savefig(path_name * "/figures/RMSE_traj_$(trial_name).pdf")
	else
		savefig(path_name * "/figures/RMSE_traj_$(trial_name).pdf")
	end
...
function plot_trace_Σ_history(trace_hist, trial_name, use_ssh_dir)
	...
	if use_ssh_dir
		savefig(path_name * "/figures/trace_traj_$(trial_name).pdf")
	else
		savefig(path_name * "/figures/trace_traj_$(trial_name).pdf")
	end
end
