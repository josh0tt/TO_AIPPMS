...
function solver_test_RoverPOMDP(pref::String; number_of_sample_types::Int=10, map_size::Tuple{Int, Int}=(10,10), seed::Int64=1234, num_graph_trials=50, total_budget = 100.0, use_ssh_dir=false, plot_results=true)
	...
	if use_ssh_dir
		writedlm( path_name * "/figures/rmse_hist_gcb.csv",  rmse_hist_gcb, ',')
		writedlm( path_name * "/figures/rmse_hist_basic.csv",  rmse_hist_basic, ',')
		writedlm( path_name * "/figures/trace_hist_gcb.csv",  trace_hist_gcb, ',')
		writedlm( path_name * "/figures/trace_hist_basic.csv",  trace_hist_basic, ',')
	else
		# writedlm( path_name * "/figures/rmse_hist_gcb.csv",  rmse_hist_gcb, ',')
		# writedlm( path_name * "/figures/rmse_hist_basic.csv",  rmse_hist_basic, ',')
		# writedlm( path_name * "/figures/trace_hist_gcb.csv",  trace_hist_gcb, ',')
